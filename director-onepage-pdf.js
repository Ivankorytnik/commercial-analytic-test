(function(){
  ...
  function activeBlockers(){
    return readJson('atom-blockers',[]).filter(x=>!['Решен','Закрыт'].includes(String(x?.status||'')));
  }
  ...
  function getData(){
    ...
    try{projectProgress=rules?.projectProgress?.() ?? core?.projectProgress?.() ?? parseInt(document.getElementById('header-progress')?.textContent||'0',10)||0;}catch{}
    ...
    const currentStages=stages
      ...
      .sort((a,b)=>(Number(b.problem)-Number(a.problem))||(b.progress-a.progress))
      .slice(0,4);
    const nextStages=stages
      .filter(s=>s.progress<100&&!currentStages.some(c=>c.id===s.id))
      .sort((a,b)=>a.id-b.id)
      .slice(0,3);
  ...
  function buildSheet(d){
    ...
    root.innerHTML=`
      <div class="dop-head">
        <div><div class="dop-brand">АТОМ</div><div class="dop-eyebrow">COMMERCIAL ANALYTICS · B2B</div></div>
        <div class="dop-head-right"><b>ONE-PAGE ДЛЯ КОММЕРЧЕСКОГО ДИРЕКТОРА</b><span>Срез на ${dateLabel(Date.now())}</span></div>
      </div>
      ...
      <div class="dop-main-grid">
        ...
        <section class="dop-panel"><h2>Готовность контура</h2>
          <div class="dop-metric"><span>Владельцы / RACI</span><b>${d.owners.ready||0}/${d.owners.total||0}</b></div>
          <div class="dop-metric"><span>Источники данных</span><b>${d.sources.ready||0}/${d.sources.total||0}</b></div>
          <div class="dop-metric"><span>Data Dictionary</span><b>${d.dictionary.ready||0}/${d.dictionary.total||0}</b></div>
          <div class="dop-flow">Источник лида <i>→</i> Сайт / Метрики <i>→</i> ELMA <i>→</i> Альфа-Авто <i>→</i> 1С <i>→</i> DWH <i>→</i> BI</div>
        </section>
      </div>
      <div class="dop-bottom-grid">
        <section class="dop-panel focus"><h2>Ближайший управленческий фокус</h2><ol>${d.focus.map(x=>`<li>${esc(x)}</li>`).join('')}</ol></section>
        <section class="dop-panel next"><h2>Следующие этапы</h2>${d.nextStages.length?d.nextStages.map(s=>`<div class="dop-next-row"><span>${esc(s.name)}</span><b>${s.progress}%</b></div>`).join(''):'<div class="dop-ok">Все этапы закрыты</div>'}</section>
      </div>
  ...
  function ensureStyles(){
    ...
    style.textContent=`
      ...
      .dop-foot{height:46px;padding:0 28px;background:#fff;border-top:1px solid #dbe5e5;display:flex;align-items:center;justify-content:space-between;gap:20px;font-size:8.5px;color:#66797a}
      .director-pdf-nav{display:block;width:100%;text-align:left;border:0;background:#102526;padding:12px 14px;margin:4px 0;border-radius:8px;color:#fff;cursor:pointer;font:700 13px Arial,Helvetica,sans-serif}.director-pdf-nav:hover{background:#173f40}.director-pdf-nav:disabled{opacity:.55;cursor:wait}
      @media(max-width:900px){.director-pdf-nav{min-width:220px;margin-right:4px}}
    `;
  ...
  function loadScript(url){
    return new Promise((resolve,reject)=>{
      ...
      const s=document.createElement('script');s.src=url;s.async=true;s.dataset.pdfLib='1';s.onload=()=>{s.dataset.loaded='1';resolve();};s.onerror=()=>reject(new Error(`Не удалось загрузить ${url}`));document.head.appendChild(s);
    });
  }
  ...
  async function loadAny(urls,check){
    if(check())return;
    let last;
    for(const url of urls){try{await loadScript(url);if(check())return;}catch(e){last=e;}}
    throw last||new Error('PDF-библиотека недоступна');
  }

  async function ensureLibraries(){
    await loadAny(LIBS.html2canvas,()=>typeof window.html2canvas==='function');
    await loadAny(LIBS.jspdf,()=>Boolean(window.jspdf?.jsPDF));
  }

  async function waitForData(limit=2600){
    const start=Date.now();
    while(Date.now()-start<limit){if(window.ATOM_CORE&&window.ATOM_REQUIREMENT_PROGRESS_RULES) return;await new Promise(r=>setTimeout(r,100));}
  }

  async function generate(){
    const btn=document.getElementById('director-onepage-pdf-btn');
    ...
    try{
      ...
      pdf.setProperties({title:'АТОМ - Коммерческая аналитика - One-page для коммерческого директора',subject:'Статус проекта коммерческой аналитики',creator:'ATOM Commercial Analytics'});
      pdf.save(`ATOM_Commercial_Analytics_OnePage_${fileDate()}.pdf`);
      window.dispatchEvent(new CustomEvent('atom-director-onepage-generated',{detail:{version:VERSION,generatedAt:new Date().toISOString()}}));
    }catch(e){
      console.error('director onepage pdf failed',e);
      alert('Не удалось сформировать PDF. Проверьте интернет-соединение и повторите.');
    }finally{
      sheet?.remove();
      if(btn){btn.disabled=false;btn.textContent=old||'One-page PDF для директора';}
    }
  }
  ...
  function bind(){
    ensureStyles();
    const btn=document.getElementById('director-onepage-pdf-btn');
    if(btn&&!btn.dataset.bound){btn.dataset.bound='1';btn.addEventListener('click',generate);}
  }

  bind();
  window.addEventListener('atom-sync-ready',bind);
  window.ATOM_DIRECTOR_ONEPAGE={version:VERSION,generate,getData};
})();
