(function(){
  ...
  function styles(){
    ...
    const s=document.createElement('style');s.id='requirements-enhanced-css';s.textContent=`
      ...
      .req-enh-table{min-width:1420px}.req-enh-overdue{background:#fff0ed!important}.req-enh-overdue td{border-bottom-color:#f2c5bc!important}.req-enh-not-actual{background:#f3f5f5!important;color:#7b8889}.req-enh-not-actual td{opacity:.76}.req-enh-not-actual select,.req-enh-not-actual textarea{opacity:1}.req-enh-team-inactive td:first-child{box-shadow:inset 3px 0 0 #c4cece}.req-enh-team-note{display:block;margin-top:3px;font-size:8px;color:#7b8a8b}.req-enh-due{font-weight:700;white-space:nowrap}.req-enh-due.overdue{color:#b23c2f}.req-enh-due.done{color:#25715a}.req-enh-due.na{color:#6d7b7c}.req-enh-period{white-space:nowrap}.req-enh-add{display:grid;grid-template-columns:minmax(220px,1fr) 220px 230px auto;gap:8px;align-items:end;background:#fff;border:1px solid var(--line);border-radius:10px;padding:10px}.req-enh-add .pa-field input,.req-enh-add .pa-field select{width:100%;box-sizing:border-box;padding:8px;border:1px solid var(--line);border-radius:7px;background:#fff;font:inherit;font-size:11px}.req-enh-raci-na{background:#f3f5f5!important;color:#7b8889}.req-enh-raci-na td{opacity:.75}.req-enh-raci-na select,.req-enh-raci-na textarea{opacity:1}
      @media(max-width:900px){.req-enh-add{grid-template-columns:1fr}}
  ...
  function requirementOverdue(reqOrId){const req=typeof reqOrId==='string'?core().requirement(reqOrId):reqOrId;if(!req||isNotActual(req.id))return false;return original.requirementOverdue?original.requirementOverdue(reqOrId):false}
  function requirementProblem(reqOrId){const req=typeof reqOrId==='string'?core().requirement(reqOrId):reqOrId;if(!req||isNotActual(req.id))return false;return original.requirementProblem?original.requirementProblem(reqOrId):false}
  ...
  function patchCore(){
    ...
    ['requirements','requirementsByStage','requirementProgress','requirementOverdue','requirementProblem','teamSummary','linkedSummary','stageSummary','projectProgress','dodEvaluation','reconcile'].forEach(k=>original[k]=c[k]?.bind(c));
    if(!original.requirements||!original.reconcile)return false;
  ...
  function renderRequirements(){
    ...
    styles();
    const rows=displayRows(),teams=c.teams?.()||[];
  ...
  function updateCustomStage(id,stageId){
    const key=`atom-core-requirement-meta-${id}`;let m=null;try{m=JSON.parse(localStorage.getItem(key)||'null')}catch{}if(!m)return;m.stageId=Number(stageId);localStorage.setItem(key,JSON.stringify(m));window.dispatchEvent(new CustomEvent('atom-core-data-changed',{detail:{type:'requirement-stage',id}}));core().reconcile();
  }
  ...
  document.addEventListener('click',e=>{
    if(e.target.closest('#req-enh-add')){const text=document.getElementById('req-enh-new-text')?.value.trim(),team=document.getElementById('req-enh-new-team')?.value,stage=Number(document.getElementById('req-enh-new-stage')?.value||1);if(!text)return alert('Укажите, что нужно получить');if(!team)return alert('Выберите команду');core().addRequirement(team,text,stage);core().reconcile();renderRequirements();return;}
